"""Reference solutions for Prefix Cache Hit Rate."""
...
def reusable_full_blocks(common_prefix_tokens, block_size):
    ...
    return common_prefix_tokens // block_size
...
def block_hit_rate(reused_blocks, requested_prompt_tokens, block_size):
    ...
    requested_blocks = requested_prompt_tokens // block_size
    if requested_blocks == 0:
        return 0.0
    return reused_blocks / requested_blocks
...
def token_equivalent_hit_rate(reused_blocks, requested_prompt_tokens, block_size):
    ...
    if requested_prompt_tokens == 0:
        return 0.0
    return (reused_blocks * block_size) / requested_prompt_tokens


def best_prefix_cache_match(request_tokens, cached_prompts, block_size):
    """Return (best_index, reused_blocks) for the cached prompt with the best full-block prefix."""
    best_index = -1
    best_blocks = 0
    for index, cached in enumerate(cached_prompts):
        common = common_prefix_length(request_tokens, cached)
        blocks = reusable_full_blocks(common, block_size)
        if blocks > best_blocks:
            best_index = index
            best_blocks = blocks
    return best_index, best_blocks
