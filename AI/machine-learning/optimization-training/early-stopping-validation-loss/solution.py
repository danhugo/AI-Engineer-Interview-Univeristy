"""Reference solutions for Early Stopping with Validation Loss."""

import numpy as np


def _improved(value, best, mode, min_delta):
    if mode == "min":
...
def best_epoch_and_value(values, mode="min", min_delta=0.0):
    ...
    for epoch, value in enumerate(values[1:], start=1):
        value = float(value)
        if _improved(value, best_value, mode, min_delta):
            best_epoch = epoch
            best_value = value
...
def epochs_since_best(values, mode="min", min_delta=0.0):
    ...
    best_epoch, _ = best_epoch_and_value(values, mode=mode, min_delta=min_delta)
    return len(values) - 1 - best_epoch


def should_stop_early(values, patience, mode="min", min_delta=0.0, start_from_epoch=0):
    """Return True if training should stop."""
    values = np.asarray(values, dtype=float)
    if values.size <= start_from_epoch:
        return False
    monitored = values[start_from_epoch:]
    return epochs_since_best(monitored, mode=mode, min_delta=min_delta) >= patience
...
def restore_best_weight(weights_by_epoch, values, mode="min", min_delta=0.0):
    ...
    best_epoch, _ = best_epoch_and_value(values, mode=mode, min_delta=min_delta)
    return np.array(weights_by_epoch[best_epoch], copy=True)
